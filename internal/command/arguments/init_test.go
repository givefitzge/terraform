...
func TestParseInit_stateProviderLockFile(t *testing.T) {
	...
	// Create an existing file with an incorrect extension to test with
	invalidLockFileName := ".terraform.lock.txt"
	...
	testCases := map[string]struct {
		...
		"error when -state-provider-lock-file references a file that has the wrong extension": {
